...
import { MonsterSchema, MonsterFirestoreInterface } from "@/data/schemas/Monster";
...
export class MonsterBase extends MonsterSchema {
    ...
    getUniqueInvestigators(positionSet: Set<number>, investigators: InvestigatorFirestore[]) {
    ...
    incrementAttacksComplete() {
        if (this.checkIfAttacksReached()) throw new MonsterError("Total Attacks Reached");
        this.attacks_complete += 1;
    }
    baseAttack(sessionId: string) {
        try {
            if (this.checkIfAttacksReached()) throw new MonsterError("Total Attacks Reached");
            const diceRoll = roller.roll(sessionId, `${this.damage} + ${this.damage_bonus}`)
            return diceRoll;
        }
        catch (error) {
