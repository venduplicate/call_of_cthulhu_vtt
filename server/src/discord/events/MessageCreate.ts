...
module.exports = {
    name: "messageCreate",
    once: false,
    async execute(interaction: Message, logger: winston.Logger, sonic: SonicEmitter) {
        if (interaction.author.bot) return;
        if (!interaction.content.match(diceRegex) && !interaction.content.match(mathRegex)) return;
